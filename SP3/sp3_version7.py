...
metadata = {
    'protocolName': 'SP3 Sample Preparation v7.0',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    def wash_func(odds, well, i):
        ...
        movePoint = Point(-2,-2,1) if odds else Point(2,-2,1) # this is the point we will move to for the odd and even wells respectively, to account for bead position
    ...
    for i in range(3): # we do 3 washes with etoh80
        ...
        if evens != []: # if there are even wells to process
            for j, well in enumerate(evens):
                wash_func(odds = False, well = well, i = j)
    ...
    if odds!= []: # if there are odd wells to process
        for well in odds:
            p300.transfer(etoh80_vol * 1.1, well.bottom().move(Point(-2,-2,1)), waste.top(), new_tip = 'always', air_gap = 10,
