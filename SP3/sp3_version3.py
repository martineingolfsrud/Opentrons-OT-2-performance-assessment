# ATTEMPT 3 at building the sp3 protocol step by step it will begin as an improved copy of katarinas sp3_v3 but not be hard-coded
# but instead utilize the parameter functionality to make it more flexible
from opentrons import protocol_api
from opentrons.types import Point

metadata = {
    'protocolName': 'SP3 Sample Preparation v3.0',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    p20.transfer(protocol.params.bead_vol, beads, magwells, new_tip = 'always')
    ...
    if odds != []:
        ...
        p300.transfer((protocol.params.sample_vol + etoh100_vol *1.1), [well.bottom().move(Point(-2,-2,1)) for well in odds], waste.top(), 
                        new_tip = 'once', air_gap = 10, blow_out = True, blowout_location = 'destination well')
    if evens != []:
        # evens
