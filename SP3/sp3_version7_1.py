# ATTEMPT 7 (major) to implement sp3 washing and binding, including more reuse of tips during washes, to minimize tip usage.
# the three repeat washes requires 7 tips per sample, which is a lot when we want to process a significant number of samples.

from opentrons import protocol_api
from opentrons.types import Point
metadata = {
    'protocolName': 'SP3 Sample Preparation v7.1',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    #!!! we need to consider total volume in well (sample, beads, red/alc, water AND beads for a total of 52.5 ul)
    sample_vol = 52.5
    ...
    tips300_2 = protocol.load_labware('opentrons_96_tiprack_300ul', 6) # for the second p300 tips if needed
    ...
    p300 = protocol.load_instrument('p300_single_gen2', 'right', tip_racks=[tips300, tips300_2])
    ...
    def add_beads(well):
        ...
        p300.transfer(etoh100_vol, etoh100.bottom(40), well
    ...
    def wash_func(odds, well, i):
        ...
        # 1: removing supernatant #* this transfer command must be rewritten like the one below to reuse tips for each wash
        p300.flow_rate.aspirate = 25
        p300.pick_up_tip(reuse_300.wells()[i+1]) # pick up the first tip for reuse during washes
        p300.aspirate(etoh80_vol*1.1, well.bottom().move(movePoint)) 
        p300.dispense(location=waste.top())
        p300.blow_out(waste.top())
        p300.drop_tip(reuse_300.wells()[i+1]) # drop the tip back #? hopefully this is fine and does not cause contamination, if this works as intended, each well has its own tip

        #2: adding etoh80 
        p300.flow_rate.aspirate = 94  # reset to default aspirate speed for the wash step
        p300.pick_up_tip(reuse_300.wells_by_name()['A1'])
        p300.aspirate(etoh80_vol, etoh80.bottom(40))
        p300.dispense(location=well.top())
        p300.blow_out(well.top())
        p300.drop_tip(reuse_300.wells_by_name()['A1']) 

    def mix_func(wells):
        # mixes after ALL wells have been washed
        for well in wells:
            p300.pick_up_tip()
            p300.mix(10, etoh80_vol if mix_vol_wash < 300 else 300, well.bottom())
            p300.blow_out(well.bottom())
            p300.drop_tip()
    ...
    if odds!= [] and len(odds)>=8: # if there are odd wells to process
    ...
    if evens != [] and len(evens)>=8: # if there are even wells to process
        ...
        #bead_agg() #! this should be unecessary, as we will not be adding any more beads after this :)
    ...
    for i in range(3): # we do 3 washes with etoh80
        ...
        if odds!= []: # if there are odd wells to process
            for j, well in enumerate(odds):
                protocol.comment(f"Odds: {j}-{well.well_name}")
                wash_func(odds = True, well = well, i = j)
        
        if evens != []: # if there are even wells to process
            #! added +8 to the index, because enumerate obvs starts at 0 again when counting the evens, despite already counting to 8 for the odds!
            #! during testing this lead to A2 also using B1 as its reuse tip, which is already used by A1 !
            for j, well in enumerate(evens):
                wash_func(odds = False, well = well, i = j+8)
        
        magdeck.disengage() # need to disengage to mix the samples
        mix_func(magwells) # mixing after all wells have been washed
    ...
    #! removing the air gap here, it seems like that is partly responsible for drawing up beads during aspiration = bad :(
    p300.flow_rate.aspirate = 25
    
    if odds!= []: # if there are odd wells to process
        for well in odds:
            p300.transfer(etoh80_vol * 1.1, well.bottom().move(Point(-1,0,1)), waste.top(), new_tip = 'always', 
