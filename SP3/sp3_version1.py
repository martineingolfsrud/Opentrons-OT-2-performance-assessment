# ATTEMPT 1 at building the sp3 protocol step by step it will begin as an improved copy of katarinas sp3_v3 but not be hard-coded
# but instead utilize the parameter functionality to make it more flexible
from opentrons import protocol_api
from opentrons.types import Point

metadata = {
    'protocolName': 'SP3 Sample Preparation',
...
def add_parameters(parameters: protocol_api.ParameterContext):
    ...
    parameters.add_float(display_name='Sample Volume', variable_name='sample_vol', default = 30.0,
                        description='Volume of each sample in uL.',  unit='uL', minimum = 20.0, maximum = 50.0)
    ...
    parameters.add_float(display_name='Bead Volume (each sample)', variable_name='bead_vol', default = 2.0,
    ...
    parameters.add_str(display_name = 'Bead Slot', variable_name = 'bead_well', default = '0', 
                        choices = tube_rack)
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    protocol.comment(f'Using wells: {[well.display_name for well in magwells]}')
    ...
    p20.transfer(protocol.params.bead_vol, beads, magwells
                , mix_after = (5, protocol.params.bead_vol), new_tip = 'always')
    ...
    mix_vol = protocol.params.sample_vol + etoh100_vol
    p300.transfer(etoh100_vol, etoh100.bottom(70), magwells
    ...
    p300.transfer((protocol.params.sample_vol + etoh100_vol *1.1), [well.bottom(1) for well in magwells], waste.top(), 
                    new_tip = 'once', air_gap = 10, blow_out = True, blowout_location = 'source well')
    ...
    for i in range(3):  # Repeat wash 3 times
        ...
        p300.transfer(protocol.params.etoh80_vol, etoh80.bottom(60), [well.bottom() for well in magwells]
        ...
        #? modify here after running test with types.Point - which way does it move? - have determined this now, see image from oct. 10
        #? modify Point object to be lower in the well, it does not pick everything up! try with the standard 1mm(?)
        p300.flow_rate.aspirate = 25  # Slow down aspirate speed to avoid disturbing beads
        p300.transfer(protocol.params.etoh80_vol * 1.1, [well.bottom().move(Point(-2,-2,1)) for well in magwells], waste.top()
                        , new_tip = 'once', air_gap = 10,
                        blow_out = True, blowout_location = 'destination well')
