# ATTEMPT 6 at programming the sp3 protocol - I am attempting a new approach in which I create functions for the repetitive washing steps
# where we know we need to alter the order of things to avoid the beads drying out. I am also modifying the bead addition itself, 
# again to avoid the beads drying out. 

from opentrons import protocol_api
from opentrons.types import Point
metadata = {
    'protocolName': 'SP3 Sample Preparation v6.0',
...
def add_parameters(parameters: protocol_api.ParameterContext):
    ...
    #? number of samples
    parameters.add_int(display_name = 'Number of Samples', variable_name='num_samples', default = 16, 
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    tips300_2 = protocol.load_labware('opentrons_96_tiprack_300ul', 4) # for the second p300 tips if needed
    ...
    def wash_func(odds, well):
        ...
        p300.transfer(etoh80_vol*1.1, well.bottom().move(movePoint), waste.top(), new_tip = 'always', air_gap = 10, blow_out = True, blowout_location = 'destination well') 
        #2: adding etoh80
        p300.flow_rate.aspirate = 94  # reset to default aspirate speed for the wash step
        p300.pick_up_tip(reuse_300.wells()[0])
        p300.aspirate(etoh80_vol, etoh80.bottom(40))
        p300.dispense(location=well.top())
        p300.blow_out(well.top())
        p300.drop_tip(reuse_300.wells()[0]) 
    ...
    for i in range(3): # we do 3 washes with etoh80
        ...
        if odds!= []: # if there are odd wells to process
            for well in odds:
                wash_func(odds = True, well = well)
        
        if evens != []: # if there are even wells to process
            for well in evens:
                wash_func(odds = False, well = well)
